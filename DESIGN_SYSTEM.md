...
**Zwevende-pil-patroon (geen achtergrondbalk).** Beide onderstaande elementen tonen alleen de pil-knop(pen) zelf, `position: fixed` en gecentreerd, zónder vlakke achtergrondbalk erachter — de onderliggende content (Calendar, set-lijsten, kaarten, ...) scrollt er gewoon doorheen en blijft zichtbaar tot aan de werkelijke schermrand; de pillen zweven erboven met een eigen `box-shadow` voor diepte. De wrapper-`<div>` van elke pil-rij is zelf `pointer-events: none` (onzichtbaar, geen achtergrond, geen klikgebied) — alleen de pil-knop(pen) zelf zijn `pointer-events: auto`, zodat je rond de pil nog gewoon door de content heen kunt scrollen/tikken.
  - **Geen zichtbare naad**: `.screen` reserveert de ruimte voor deze pillen niet meer via een kortere `bottom`-positionering (die de schermachtergrond fysiek liet stoppen vóór de echte onderrand, wat vroeger onzichtbaar was omdat de opake balk die strook opvulde) maar via `padding-bottom: var(--ai-bar-space)` **op het `.screen`-element zelf**. Omdat `.screen` daardoor weer de volle viewporthoogte beslaat (`inset: 0`), loopt de eigen achtergrondkleur van elk scherm (licht `--bg`, of donker bij `.plan-screen`/`.screen-placeholder`) altijd door tot de echte rand — geen kleurmismatch meer onder de pil, voor geen enkel scherm, zonder dat elk scherm z'n scrollbare content apart hoeft aan te passen.
...
- **Globale AI Coach-pil** (`#global-ai-bar` = de positionerings-wrapper, knop `.global-ai-bar-pill` / `#btn-global-ai-coach`) — zwevend onderaan de viewport, boven de Android-systeembalk (`padding-bottom: calc(var(--safe-bottom) + 16px)`). Groene 135°-gradient-achtergrond (`linear-gradient(135deg, var(--color-ai-coach-gradient-start), var(--color-ai-coach-gradient-end))`), `border-radius: 22px`, en een gecombineerde schaduw voor diepte + accent: `box-shadow: 0 4px 14px rgba(0,0,0,.25), 0 3px 10px var(--color-ai-coach-glow)` (een neutrale donkere schaduw voor het zwevende effect, plus de groene gloed erbovenop). Sparkle-icoon en "AI Coach"-tekst zijn beide wit (`#fff`) en bold.
  ...
  - **Hoogte wordt gereserveerd via `--ai-bar-height` (64px) en `--ai-bar-space` (`--ai-bar-height` + `--safe-bottom`)** — deze tokens heten nog naar de oude balk-implementatie maar betekenen nu "gereserveerde zone voor de zwevende pil"; niet hernoemd om onnodige churn te voorkomen. **Nieuwe schermen hoeven hier niets extra's voor te doen**, zolang ze de standaard `.screen`-class gebruiken.
...
- **Dag-acties-pillen** (`#day-actions-row` = de positionerings-wrapper, "Recap" + "Comment", `.day-action-pill`) — zelfde zwevende-pil-patroon, maar **niet écht globaal**: enkel zichtbaar wanneer `#screen-fitness-tracker` zowel `.active` als `.has-day-actions` heeft (CSS-sibling-selector `#screen-fitness-tracker.active.has-day-actions ~ .day-actions-row`). `.has-day-actions` wordt in `renderHome()` (app.js) gezet zodra de huidige dag oefeningen bevat, en verwijderd in de lege-staat. Wit/rand-stijl (`border: 1px solid var(--border)`, `background: var(--surface)`) met een neutrale schaduw (`box-shadow: 0 3px 10px rgba(0,0,0,.15)`) — bewust géén groene gradient, zodat 'm nooit met de AI Coach-pil verward wordt. Zweeft met `bottom: var(--ai-bar-space)` — dus direct boven de zone van de AI Coach-pil. Hoogte gereserveerd via `--day-actions-height` (56px): wanneer deze pillen zichtbaar zijn, krijgt `#screen-fitness-tracker` zélf extra bottom-padding: `#screen-fitness-tracker.has-day-actions { padding-bottom: calc(var(--ai-bar-space) + var(--day-actions-height)); }` — bewust alléén op dit scherm, niet globaal via `--ai-bar-space` zelf. Een volgend scherm dat een eigen extra pil-rij nodig heeft, volgt dit patroon: eigen `.has-*`-klasse + eigen `padding-bottom: calc(var(--ai-bar-space) + eigen-hoogte)`.
